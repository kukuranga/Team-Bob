...
public class SnowMovement : MonoBehaviour
{
      ...
      private void checkIfMoving()
      {
            ...
            if(distance <= 1)
            {
                  ...
                  moveSnowBlocks(this.gameObject,determinVertical());
            }
      }
      ...
      public void checkPlayerPositionStatus()
      {
            ...
            else if (player.GetComponent<Rigidbody2D>().transform.position.x != rigidbody.transform.position.x && player.GetComponent<Rigidbody2D>().transform.position.y != rigidbody.transform.position.y)
            {
                  this.directionMovable = 0;
            }
      }
      ...
      public List<GameObject> findConnectedSnowBlocks(List<GameObject> snowBlocks, GameObject snowBlock, bool isVertical)
      {
            for (int i = 0; i < Snow.Length; i++)
            {
                  GameObject adjacentBlock = Snow[i].gameObject;
                  if (snowBlock != adjacentBlock && isConnected(snowBlock, adjacentBlock, isVertical) && adjacentBlock.GetComponent<BoxCollider2D>().isTrigger == true)
                  {
                        if (snowBlocks.IndexOf(adjacentBlock) == -1)
                        {
                              snowBlocks.Add(adjacentBlock);
                              //print("added " + adjacentBlock.gameObject.name);
                              findConnectedSnowBlocks(snowBlocks, adjacentBlock, isVertical);
                        }
                  }
            }
            return snowBlocks;
      ...
      private Boolean isConnected(GameObject alpha, GameObject beta, bool isVertical)
      {
            if (moveable == true)
            {
                  //print("A=" + alpha + ",B=" + beta);
                  if (alpha.transform.position.x - 1 == beta.GetComponent<Rigidbody2D>().transform.position.x &&
                      alpha.transform.position.y == beta.GetComponent<Rigidbody2D>().transform.position.y && isVertical == false)
                  {
                        return true;
                  }
                  else if (alpha.transform.position.x + 1 == beta.GetComponent<Rigidbody2D>().transform.position.x &&
                      alpha.transform.position.y == beta.GetComponent<Rigidbody2D>().transform.position.y && isVertical == false)
                  {
                        return true;
                  }
                  else if (alpha.transform.position.x == beta.GetComponent<Rigidbody2D>().transform.position.x &&
                      alpha.transform.position.y + 1 == beta.GetComponent<Rigidbody2D>().transform.position.y && isVertical == true)
                  {
                        return true;
                  }
                  else if (alpha.transform.position.x == beta.GetComponent<Rigidbody2D>().transform.position.x &&
      ...
      public void moveSnowBlocks(GameObject Snowblock, bool isVertical)
      {
            if (moveable == true)
            {
                  ...
                  findConnectedSnowBlocks(snowBlocks, Snowblock, isVertical);
      ...
      private bool seeIfSnowCanMoveMoved(GameObject objectCheckingAgainst, GameObject snow, int direction)
      {
            if (direction == 1)
            {
                  if (snow.transform.position.x == objectCheckingAgainst.transform.position.x - 1 && snow.transform.position.y == objectCheckingAgainst.transform.position.y)
                  {
                        return false;
                  }
            }
            ...
            {
                  if (snow.transform.position.x == objectCheckingAgainst.transform.position.x && snow.transform.position.y == objectCheckingAgainst.transform.position.y + 1)
                  {
                        return false;
                  }
            }
      ...
      public bool hasSnowBeside(int direction)
      {
            for (int i = 0; i < Snow.Length; i++)
            {
                  if (direction == 1)
                  {
                        if (transform.position.x == Snow[i].transform.position.x + 1 && transform.position.y == Snow[i].transform.position.y)
                        {
                              return true;
                        }
                  }
                  ...
                  {
                        if (transform.position.y == Snow[i].transform.position.y - 1 && transform.position.x == Snow[i].transform.position.x)
                        {
                              return true;
                        }
                  }
